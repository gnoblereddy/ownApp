...
var createProductDb = require('./models/createProductSchema');
...
router.post('/createProduct', function (req, res, next) {
    let data = new createProductDb(req.body);
    data.save(function (err, data) {
        if (err) {
            Response.errorResponse(err.message, res);
        } else {
            return res.status(200).json({ result: data, message: 'Create Product Successfull', success: 1, statusCode: 200, status: true });
        }
    })
})
